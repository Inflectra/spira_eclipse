...
public class RichText extends Composite
{
		...
		while (start < end)
		{
			...
			if (ranges != null && ranges.length > 0)
			{
				for (int i = 0; i < ranges.length; i++)
				{
					...
					builder.append(plainText.substring(ranges[i].start, ranges[i].start + ranges[i].length));
	...
	private Integer[] getLineBreaks()
	{
		...
		{
			int br = styledText.getText().indexOf(styledText.getLineDelimiter(), lastIdx);
	...
	{
		if ((event.keyCode == SWT.ARROW_LEFT) || (event.keyCode == SWT.ARROW_UP) || (event.keyCode == SWT.ARROW_RIGHT) || (event.keyCode == SWT.ARROW_DOWN))
		{
			updateStyleButtons();
		}
	}
	...
	private ToolBar createToolBar(Composite parent)
	{
		...
		boldBtn.setImage(SpiraTeamImages.IMG_BOLD.createImage());
		boldBtn.setToolTipText(Messages.boldBtn_tooltipText);
		boldBtn.addSelectionListener(new FontStyleButtonListener(FontStyle.BOLD));

		italicBtn = new ToolItem(toolBar, SWT.CHECK);
		italicBtn.setImage(SpiraTeamImages.IMG_ITALIC.createImage());
		italicBtn.setToolTipText(Messages.italicBtn_tooltipText);
		italicBtn.addSelectionListener(new FontStyleButtonListener(FontStyle.ITALIC));

		underlineBtn = new ToolItem(toolBar, SWT.CHECK);
		underlineBtn.setImage(SpiraTeamImages.IMG_UNDERLINE.createImage());
		underlineBtn.setToolTipText(Messages.underlineBtn_tooltipText);
		underlineBtn.addSelectionListener(new FontStyleButtonListener(FontStyle.UNDERLINE));

		strikeThroughBtn = new ToolItem(toolBar, SWT.CHECK);
		strikeThroughBtn.setImage(SpiraTeamImages.IMG_STRIKE_THROUGH.createImage());
		strikeThroughBtn.setToolTipText(Messages.strikeThroughBtn_tooltipText);
		strikeThroughBtn.addSelectionListener(new FontStyleButtonListener(FontStyle.STRIKE_THROUGH));

		new ToolItem(toolBar, SWT.SEPARATOR);

		ToolItem cutBtn = new ToolItem(toolBar, SWT.PUSH);
		cutBtn.setImage(SpiraTeamImages.IMG_CUT.createImage());
		cutBtn.setToolTipText(Messages.cutBtn_tooltipText);
		...
		copyBtn.setImage(SpiraTeamImages.IMG_COPY.createImage());
		copyBtn.setToolTipText(Messages.copyBtn_tooltipText);
		...
		pasteBtn.setImage(SpiraTeamImages.IMG_PASTE.createImage());
		pasteBtn.setToolTipText(Messages.pasteBtn_tooltipText);
		...
		eraserBtn.setEnabled(false);
		eraserBtn.setImage(SpiraTeamImages.IMG_ERASER.createImage());
		eraserBtn.setToolTipText(Messages.eraserBtn_tooltipText);
